...
async function store (message) {
  ...
  const document = message.document || message.voice || message.video ||
    message.video_note || message.audio || message.animation
  ...
  if (document) {
    ...
    if (document.thumb) {
      ...
      const thumbFileName = ['thumb', fileName].join('_')
