...
async function store (message, edited = false) {
  ...
  const messageValues = [chatId, messageId, dateCreated, dateEdited, edited, replyTo, userId, text, data]
  ...
  if (entities && entities.length) {
    ...
    for (const hashtag of hashtags) {
      const hashtagQuery = `INSERT INTO
        hashtags(hashtag, chat_id, message_id)
        VALUES ($1, $2, $3)`
      const hashtagValues = [hashtag, chatId, messageId]
